...
public class Locatarios {
    ...
    public Locatario cadastrarLocatario(@RequestBody Locatario locatario){
        ...
        } else if (locatario.getVagas().getLocal() == null || locatario.getVagas().getLocal().equals(this)){
            throw new RequisicaoInvalida("Você deve informar o local e ele não pode ser igual a um existente");
        } else if (locatario.getVagas().getAndar() > 11 || locatario.getVagas().getAndar() < 1){
            throw new RequisicaoInvalida("Andar deve ser entre 1 e 10, obrigado!");
        } else {
    ...
    public Pagamento cadastrarPagamento(@PathVariable int idLocatario, @RequestBody Pagamento pagamento){
        Locatario locatarioBanco = this.buscar(idLocatario);
